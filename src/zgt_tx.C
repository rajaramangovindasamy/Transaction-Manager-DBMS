...
bool accesslock = false;// Transaction manager object
...
void *readtx(void *arg){
  ...
  if(txptr!=NULL)
  {
       ...
       if(txptr->status==TR_ACTIVE)
      {
          zgt_v(0);
          lockstatus =txptr->set_lock(node->tid,txptr->sgno,node->obno,node->count,'S');
          printf("%d lock status",lockstatus);
          if(lockstatus==0){
                 txptr->perform_readWrite(txptr->tid,node->obno,'S');}//read operation
                
      }
       else if(txptr->status==TR_ABORT)
      {
          fprintf(logfile,"T%d\t%c \tReadTxn\n",node->tid,node->Txtype);
          fflush(logfile);
          zgt_v(0);
          txptr->print_tm();
      }
  ...
  {
        zgt_v(0);// No transaction found
        fprintf(logfile, "\n***T%d ReadTx (%c) on obno %d failed. Transaction does not exist.***\n",  node->tid, node->Txtype,node->obno);
	fflush(logfile);     
  }
...
void *writetx(void *arg){ //do the operations for writing; similar to readTx
  ...
  if(txptr!=NULL)
  {
      ...
      {
          zgt_v(0);
          lockstatus =txptr->set_lock(node->tid,txptr->sgno,node->obno,node->count,'X');
          printf("%d lock status",lockstatus);
          if(lockstatus==0)
          {
              txptr->perform_readWrite(txptr->tid,node->obno,'X'); //write operation
          }  
      }
       
      else if(txptr->status==TR_ABORT)
      {
          fprintf(logfile,"T%d\t%c \tWriteTxn\n",node->tid,node->Txtype);
          fflush(logfile);
          zgt_v(0);
  ...
  {
       zgt_v(0); //Transaction not found
      fprintf(logfile,"\n---T%d Write transaction %c on obno %d failed. No Transaction found.---\n",node->tid,node->Txtype,node->obno);
      fflush(logfile);
  }
  finish_operation(node->tid);
  pthread_exit(NULL);
...
void *do_commit_abort(long t, char status)
{
    ...
    if(txptr==NULL)
    {
        fprintf(logfile,"\n---Trasaction doesn't exists cannot commit or abort.---\n");
        fflush(logfile);
    }
    else
    {
...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count, char lockmode1){
  ...
  //if successful  return(0);
    ...
    while(accesslock==false)
    {
        zgt_p(0);
        zgt_tx *txptr =get_tx(tid1);
        zgt_hlink *listpt = ZGT_Ht->find(sgno1,obno1);
        txptr->print_tm();
        if(listpt==NULL)
        {
            ZGT_Ht->add(txptr,sgno1,obno1,lockmode1);
            accesslock=true;
            listpt=ZGT_Ht->find(sgno1,obno1);
        }
        else
        {
            if(listpt->tid==tid1)
            ...
            {
                if(get_tx(listpt->tid)->Txtype=='R'&& txptr->Txtype=='R')
                {
                    accesslock=true;
                    if(txptr->head==NULL)
                    {
                    ...
                    {
                        zgt_hlink* LinkElement=txptr->head;
                        while(LinkElement->nextp!=NULL){
                            LinkElement=LinkElement->nextp;
                        }
                        LinkElement->nextp=listpt;
                    }
                }
                else{
                    ...
                    txptr->setTx_semno(listpt->tid,listpt->tid);
                    printf("Tx %d is waiting for T%d\n",txptr->tid,listpt->tid);
                    zgt_v(0);
                    zgt_p(listpt->tid);
                }
            }
...
*/
      if (ZGT_Ht->remove(this,1,(long)temp->obno) == 1){
	   printf(":::ERROR:node with tid:%d and obno:%d was not found for deleting", this->tid, temp->obno);		// Release from hash table
	   fflush(stdout);
      }
      else {
#ifdef TX_DEBUG
